...
const Input = ({ 
  ...
  const inputBaseStyles = "w-full bg-surface-card border py-3 px-4 text-text-primary rounded-sm transition-colors placeholder:text-text-secondary/50 focus:outline-none focus:border-gold";
  ...
  return (
    <div className="w-full">
      ...
      {type === 'textarea' ? (
        <textarea rows="6" {...inputProps} className={`${inputProps.className} resize-none`} />
      ) : type === 'select' ? (
        <div className="relative">
          <select {...inputProps} className={`${inputProps.className} appearance-none`}>
