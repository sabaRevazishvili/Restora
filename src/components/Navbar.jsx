    ...
    <>
      ...
      >
        <div className="container mx-auto px-6 md:px-12 grid grid-cols-2 md:grid-cols-3 items-center">
          ...
          <div className="flex justify-start">
            <a href="#hero" className="font-serif text-2xl text-sand tracking-wide">
              Restroa
            </a>
          </div>
          ...
          <div className="flex justify-end items-center">
            <Button 
              ...
              size="sm"
              variant="secondary"
              className="hidden md:inline-block border-gold text-gold hover:bg-gold hover:text-text-inverse"
            >
              Start a Project
            </Button>
            ...
            >
              <div className="flex flex-col items-center justify-center w-full h-full">
                <span className={`block w-6 h-[1.5px] bg-current transition-all duration-300 ${mobileMenuOpen ? 'rotate-45 translate-y-[1.5px]' : '-translate-y-1'}`}></span>
                <span className={`block w-6 h-[1.5px] bg-current transition-all duration-300 ${mobileMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
                <span className={`block w-6 h-[1.5px] bg-current transition-all duration-300 ${mobileMenuOpen ? '-rotate-45 -translate-y-[1.5px]' : 'translate-y-1'}`}></span>
              </div>
            </button>
          </div>
      ...
      >
        <div className="flex flex-col items-center space-y-8">
          {[...navLinks, { name: 'Start a Project', href: '#start-project', isCTA: true }].map((link, idx) => {
            if (link.isCTA) {
              return (
                <div 
                  key={link.name}
