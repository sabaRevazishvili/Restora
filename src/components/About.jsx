...
import { stats, ABOUT_TEXT } from '../constants';

const About = () => {
  return (
    <section 
      id="about" 
      className="py-16 md:py-28 bg-surface-section"
    >
      <div className="container mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-[55%_45%] gap-16 lg:gap-24 items-center">
        {/* Left Column - Content */}
        <ScrollReveal>
          <div className="text-gold font-sans text-xs uppercase tracking-[0.2em] mb-6">{ABOUT_TEXT.eyebrow}</div>
          ...
          <div className="space-y-6 text-text-secondary font-sans text-lg mb-12">
            {ABOUT_TEXT.paragraphs.map((p, idx) => (
              <p key={idx}>{p}</p>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 pt-12 border-t border-border-subtle">
            {stats.map((stat, idx) => (
              <div key={stat.id} className={`flex flex-col ${idx !== stats.length - 1 ? 'border-r border-border-subtle' : ''}`}>
                <span className="font-serif text-3xl md:text-4xl text-gold mb-1">{stat.value}</span>
        ...
        <ScrollReveal delay={150} className="relative max-w-full">
          <div className="relative aspect-square bg-surface-card border border-gold/30 flex items-center justify-center p-8 md:p-12 shadow-[inset_0_0_80px_rgba(0,0,0,0.5)] max-w-full overflow-hidden sm:overflow-visible">
            <svg width="100%" height="100%" viewBox="0 0 100 100" className="opacity-40 max-w-full">
              <rect x="20" y="20" width="60" height="60" stroke="var(--color-gold)" strokeWidth="0.5" fill="none" />
              <rect x="10" y="10" width="60" height="60" stroke="var(--color-gold)" strokeWidth="0.5" fill="none" transform="rotate(45 50 50)" />
              <circle cx="50" cy="50" r="15" stroke="var(--color-gold)" strokeWidth="0.5" fill="none" />
              <line x1="0" y1="50" x2="100" y2="50" stroke="var(--color-gold)" strokeWidth="0.2" />
              <line x1="50" y1="0" x2="50" y2="100" stroke="var(--color-gold)" strokeWidth="0.2" />
            </svg>
